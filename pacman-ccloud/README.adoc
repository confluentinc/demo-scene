...
* *Java and Maven* - If you intend to use the AWS implementation then you will need to have https://openjdk.java.net/[Java] and https://maven.apache.org/[Maven] installed. The AWS implementation uses a serverless approach for all its layers, and the event handler that processes the events from the application is implemented as a Lambda function written in Java.

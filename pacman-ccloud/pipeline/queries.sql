...
CREATE STREAM HIGHEST_SCORE_SOURCE AS
	SELECT 'HIGHEST_SCORE_KEY' AS HIGHEST_SCORE_KEY, SCORE
	FROM USER_GAME_REKEYED;

CREATE TABLE HIGHEST_SCORE AS
	SELECT HIGHEST_SCORE_KEY, MAX(SCORE) AS HIGHEST_SCORE
	FROM HIGHEST_SCORE_SOURCE GROUP BY HIGHEST_SCORE_KEY;

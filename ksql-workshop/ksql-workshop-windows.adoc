= Workshop: Real-time SQL Stream Processing at Scale with Apache Kafka and KSQL (Windows)
:doctype: book
:sectnums:
...
----
$ docker-compose logs -f kafka|Select-String "INFO Kafka version"
kafka_1             | [2019-03-04 16:04:53,441] INFO Kafka version : 2.1.0-cp1 (org.apache.kafka.common.utils.AppInfoParser)
----
...
The ratings data is in a Kafka topic. Here we use https://github.com/edenhill/kafkacat/[`kafkacat` ] for showing the Kafka messages and detailed metadata.
...
This shows how to ingest the data using https://www.confluent.io/blog/no-more-silos-how-to-integrate-your-databases-with-apache-kafka-and-cdc[Kafka Connect and CDC].
...
In the source stream, the `ROWKEY` reflects the key of the underlying Kafka messages as set by the producer:

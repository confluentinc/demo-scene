...
I want to specify multiple columns for a key in a TABLE but can only specify one (check this) - so have to create intermediate step to build composite key first before declaring a table
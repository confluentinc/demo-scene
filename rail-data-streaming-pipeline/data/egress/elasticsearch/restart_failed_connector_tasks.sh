...
curl -s "http://localhost:28083/connectors" | \
  jq '.[]' | \
  xargs -I{connector_name} curl -s "http://localhost:28083/connectors/"{connector_name}"/status" | \
  jq -c -M '[select(.tasks[].state=="FAILED") | .name,"§±§",.tasks[].id]' | \
  grep -v "\[\]"| \
  sed -e 's/^\[\"//g'| sed -e 's/\",\"§±§\",/\/tasks\//g'|sed -e 's/\]$//g'| \
  xargs -I{connector_and_task} curl -X POST "http://localhost:28083/connectors/"{connector_and_task}"/restart"

...
You should see the record that you previously loaded into the `movies` topic, which proves that magic of KSQL which is keep your ETL pipeline live, with all the data copying between the pipes managed automatically.

Now that you have a stream with each of its records partitioned by the `movie_id` column, we can finally create our table. Execute the following command:

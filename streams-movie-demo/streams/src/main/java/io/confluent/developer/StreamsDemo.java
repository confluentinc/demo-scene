...
public class StreamsDemo {
  ...
  public static void main(String args[]) {
    ...
    final SpecificAvroSerde<Movie> movieSerde = getSpecificAvroSerde(config);
    final SpecificAvroSerde<Rating> ratingSerde = getSpecificAvroSerde(config);

    final SpecificAvroSerde<RatedMovie> ratedMovieSerde = getSpecificAvroSerde(config);
    final SpecificAvroSerde<CountAndSum> countAndSumSerde = getSpecificAvroSerde(config);
  ...
  static <T extends SpecificRecord> SpecificAvroSerde<T> getSpecificAvroSerde(final Properties envProps) {
    ...
    serdeConfig
        .put(SCHEMA_REGISTRY_URL_CONFIG, ofNullable(
            envProps.getProperty(SCHEMA_REGISTRY_URL_CONFIG)).orElse(""));
    serdeConfig
        .put(BASIC_AUTH_CREDENTIALS_SOURCE, ofNullable(
            envProps.getProperty(BASIC_AUTH_CREDENTIALS_SOURCE)).orElse(""));
    serdeConfig.
        put("schema.registry.basic.auth.user.info", ofNullable(
            envProps.getProperty("schema.registry.basic.auth.user.info")).orElse(""));
  ...
  public static Properties getStreamsConfig(String kafkaBootStrapServer,
    ...
    config.put(ROCKSDB_CONFIG_SETTER_CLASS_CONFIG, CompatibleRocksDBConfigSetter.class);

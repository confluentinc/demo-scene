...
CLI v0.10.0, Server v0.10.0 located at http://localhost:8088
...
[source,sql]
----
ksql> CREATE TABLE CUSTOMERS (CUSTOMER_ID VARCHAR PRIMARY KEY) 
...
ksql> SELECT CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, CLUB_STATUS FROM CUSTOMERS EMIT CHANGES;
...
[source,sql]
----
ksql> CREATE STREAM CUSTOMERS_STREAM (CUSTOMER_ID VARCHAR KEY) WITH (KAFKA_TOPIC='asgard.demo.CUSTOMERS', VALUE_FORMAT='AVRO');
...
curl -s -X "POST" "http://localhost:8088/query" \
     ...
     -d '{"ksql":"SELECT TIMESTAMPTOSTRING(WINDOWSTART, '\''yyyy-MM-dd HH:mm:ss'\'') AS WINDOW_START_TS,        CLUB_STATUS,       RATING_COUNT FROM   RATINGS_BY_CLUB_STATUS WHERE  CLUB_STATUS='\''bronze'\''   AND  WINDOWSTART > '$PREDICATE';"}' | json_pp
----

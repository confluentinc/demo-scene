...
_(https://cwiki.apache.org/confluence/display/KAFKA/KIP-629%3A+Use+racially+neutral+terms+in+our+codebase[KIP-629] has started to be implemented in Apache Kafka 2.6. If you are using an earlier version then you will have to use `blacklist` in place of `exclude` and `whitelist` in place of `include`)_

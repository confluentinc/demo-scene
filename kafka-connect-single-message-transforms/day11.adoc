...
curl -i -X PUT -H "Accept:application/json" \
  ...
  -d '{
      ...
      "transforms"                                   : "filterSmallOrders",
      "transforms.filterSmallOrders.type"            : "io.confluent.connect.transforms.Filter$Value",
      "transforms.filterSmallOrders.filter.condition": "$[?(@.product =~ /.*Stout/)]",
      "transforms.filterSmallOrders.filter.type"     : "include"
      }'
----

...
Anyone capable of perfectly positioning the cube using the coordinates the represents the number three will be shown in the output of a KSQL query. This query is executed against a KSQL table called *SELECTED_WINNERS*, that is the result of a join between the inbound streams (created based on the *_EVENTS* topic) and a table that holds the information of the coordinates.
...
3. You need to have the Confluent Cloud CLI installed locally. You can find more details abouy how to install it https://docs.confluent.io/current/cloud/cli/install.html[here]. Make sure to have the Confluent Cloud CLI in the path of your O.S, since the provisioning process will try to execute a command using the 'ccloud' utility.

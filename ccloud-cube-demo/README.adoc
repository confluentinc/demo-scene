...
Anyone capable of perfectly positioning the cube using the coordinates the represents the number three will be shown in the output of a KSQL query. This query is executed against a table called *SELECTED_WINNERS*, that is the result of a join between the inbound streams (created based on the *_EVENTS* topic) and a table that holds the information of the coordinates.

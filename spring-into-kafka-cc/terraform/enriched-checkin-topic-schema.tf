resource "confluent_kafka_topic" "enriched-checkin" {
  kafka_cluster {
    id = confluent_kafka_cluster.kafka_cluster.id
  }

  topic_name = "enriched-checkin-avro"
...
resource "confluent_subject_config" "enriched-checkin_value_cfg" {
  subject_name = "${confluent_kafka_topic.enriched-checkin.topic_name}-value"
  ...
  depends_on = [
    confluent_kafka_topic.enriched-checkin
  ]
}
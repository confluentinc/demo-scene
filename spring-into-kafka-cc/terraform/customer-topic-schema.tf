resource "confluent_kafka_topic" "customer_commands" {
  kafka_cluster {
    id = confluent_kafka_cluster.kafka_cluster.id
  }

  topic_name = "customer-commands-avro"
...
resource "confluent_subject_config" "customer_commands_value_cfg" {
  subject_name = "${confluent_kafka_topic.customer_commands.topic_name}-value"
  ...
  depends_on = [
    confluent_kafka_topic.customer_commands
  ]
}